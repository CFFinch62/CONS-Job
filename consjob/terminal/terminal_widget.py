...
from consjob.terminal.pty_process import PTYProcess
from consjob.config.settings import Settings
from consjob.config.scheme_detector import detect_scheme_interpreters, get_default_interpreter

class TerminalWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.settings = Settings()
        self.no_interpreter = False  # Flag to track if we have an interpreter
        ...
        # PTY Process - find an available interpreter
        cmd = self._get_interpreter_command()
        
        if cmd is None:
            # No interpreter found - set flag and show message in terminal
            self.no_interpreter = True
            self.pty = None
            self.stream.feed("No Scheme interpreter found.\r\n\r\n")
            self.stream.feed("Please install one of the following:\r\n")
            self.stream.feed("  - MIT/GNU Scheme: sudo apt install mit-scheme\r\n")
            self.stream.feed("  - GNU Guile: sudo apt install guile-3.0\r\n")
            self.stream.feed("  - Chez Scheme: sudo apt install chezscheme\r\n")
            self.stream.feed("  - Chibi-Scheme: sudo apt install chibi-scheme\r\n")
            self.stream.feed("\r\nThen restart the application or use\r\n")
            self.stream.feed("Edit -> Preferences to configure a custom path.\r\n")
        else:
        ...
        # Start the PTY if we have one
        if self.pty:
            self.pty.start()
    
    def _get_interpreter_command(self):
        """Get the interpreter command to use, checking settings and auto-detection."""
        # First check settings for a configured path
        scheme_path = self.settings.get("terminal", "lisp_path")  # Key kept for backwards compat
        if scheme_path:
            return [scheme_path]
        
        # Auto-detect an available interpreter
    ...
    def paintEvent(self, event):
        ...
        # Draw Cursor
        if self.cursor_visible and self.pty and self.pty.running:
