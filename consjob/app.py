...
from consjob.editor.tab_widget import EditorTabWidget
from consjob.terminal.terminal_widget import TerminalWidget
from consjob.browser.file_browser import FileBrowser
from consjob.config.themes import get_theme, apply_theme_to_app
from consjob.config.settings_dialog import SettingsDialog
from consjob.editor.find_replace import FindReplaceDialog

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.settings = Settings()
        self.setWindowTitle("CONS Job - Scheme Development Environment")
    ...
    def show_about(self):
        QMessageBox.about(self, "About CONS Job", 
                          "CONS Job - Scheme Development Environment v1.0\n\nA student/beginner-friendly Scheme programming environment.\n\n(c) 2025 Chuck Finch - Fragillidae Software")

    def new_file(self):
        self.editor_tabs.new_file()

    def open_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Open File", "", "Scheme Files (*.scm *.ss *.sld *.sls);;All Files (*)")
        if path:
            try:
                p = Path(path)
    ...
    def save_file_as(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save File", "", "Scheme Files (*.scm);;All Files (*)")
        if path:
    ...
    def closeEvent(self, event):
        ...
        # Terminate PTY if it exists
        if self.terminal.pty:
